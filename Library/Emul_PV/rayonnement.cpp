...
const char *cAngstromString[TAngstromCoeff_Size] = {"Bleu profond (B = 0,01)", "Bleu foncé (B = 0,02)", "Bleu pur (B = 0,035)", "Bleu (B = 0,05)",
                 "Bleu délavé (B = 0,07)", "Bleu laiteux (B = 0,15)", "Bleu voilé (B = 0,25)", "Bleu blanc (B = 0,5)"};
const char *cAngstromShortString[TAngstromCoeff_Size] = {"Bleu profond", "Bleu foncé", "Bleu pur", "Bleu",
                 "Bleu délavé", "Bleu laiteux", "Bleu voilé", "Bleu blanc"};
...
{
  return Pression_Atm(aAltitude) / (101325.0 * sin(DegToRad(aHauteurSoleil)) +
           15198.75 * pow((3.885 + aHauteurSoleil),-1.253));
}
...
{
  return 2.4 + 14.6 * cAngstromCoeff[aAngstrom] + 0.4 * (1.0 + 2.0 * cAngstromCoeff[aAngstrom]) * log(Pression_Vapeur(aTemperature,aHR));
}
...
{
  return aESol * exp(-EpaisseurRayleigh(aAltitude, aHauteurSoleil) *
                MasseAirOptique(aAltitude, aHauteurSoleil) * TroubleLinke(aAngstrom,aTemperature,aHR));
}

double RSDirect_Mer(double aESol, double aHauteurSoleil, TAngstromCoeff aAngstrom,
             double aTemperature, double aHR)  // Par défaut aHR = 0.5
{
  return aESol * exp(-TroubleLinke(aAngstrom,aTemperature,aHR) / (0.9 + 9.4 * sin(DegToRad(aHauteurSoleil))));
}
...
double Coefficient_Incidence(double aHauteurSoleil, double aAzimut, double aInclinaison, double aOrientation)
{
  ...
  result = lSin_Inc * lCos_Haut * cos(DegToRad(aOrientation - aAzimut)) +
            lCos_Inc * lSin_haut;
...
//  else
  {
    i = 1;
    while ((i<sizeof(GlobalMask)) && (GlobalMask[i].Azimut < aAzimut)) i++;
    // Interpolation linéaire : y = ax + b;  Hauteur = a.Azimut + b
    a = (GlobalMask[i].Hauteur - GlobalMask[i-1].Hauteur) / (GlobalMask[i].Azimut - GlobalMask[i-1].Azimut);
...
double Time_Equation(TDateTime aDate)
{
  ...
  C = (1.9148 * sin((357.5291 + 0.98560028 + j)*cDegRad) + 0.02 * sin((2 * (357.5291 + 0.98560028 * j))*cDegRad) + 0.0003 * sin((3 * (357.5291 + 0.98560028 * j))*cDegRad)) * 4;
  O = (-2.468 * sin((2 * (280.4665 + 0.98564736 * j))*cDegRad) + 0.053 * sin((4 * (280.4665 + 0.98564736 * j))*cDegRad) - 0.0014 * sin((6 * (280.4665 + 0.98564736 * j))*cDegRad)) * 4;
  return C + O;
}
