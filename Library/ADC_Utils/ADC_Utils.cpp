...
bool ADC_Initialize_OneShot(std::initializer_list<uint8_t> gpios)
{
	...
	timerAttachInterrupt(Timer_Tore, &do_Code);
#endif
...
#else
	return analogContinuous((uint8_t*) data(gpios), gpios.size(), CONVERSIONS_PER_PIN, 80000, &do_Code);
#endif
}
