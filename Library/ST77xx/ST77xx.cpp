...
// ST77xx Reset pin
uint16_t ST77xx_RESET_Pin;

// ST77xx Select pin
uint16_t ST77xx_CS_Pin;

// ST77xx DC pin
uint16_t ST77xx_DC_Pin;

// ST77xx BLK pin
uint16_t ST77xx_BLK_Pin;
...
//
static const uint8_t
  init_cmds1[] = {      	// Init for ST7789, part 1
  3,            		//  3 commands in list:
...
static const uint8_t
  init_cmds2[] = {      	// Init for ST7789, part 2
...
/* Division line */
static const uint8_t
  init_cmds3[] = {      	// Init for ST7789, part 3
...
static void ST77xx_SetAddressWindow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1)
{
  ...
  {
    uint8_t data[4] = {(uint8_t)(x_start >> 8), (uint8_t)(x_start & 0xFF),
  ...
  {
    uint8_t data[4] = {(uint8_t)(y_start >> 8), (uint8_t)(y_start & 0xFF),
...
{
  if ((xEnd >= ST77xx_WIDTH) || (yEnd >= ST77xx_HEIGHT))  return;
...
{
  if ((x <= 0) || (x > ST77xx_WIDTH) ||
      (y <= 0) || (y > ST77xx_HEIGHT))  return;

  ST77xx_Fill((uint16_t)(x - 1), (uint16_t)(y - 1), (uint16_t)(x + 1), (uint16_t)(y + 1), color);
}
...
void ST77xx_WriteChar(uint16_t x, uint16_t y, char ch, Font_TypeDef font, uint16_t color, uint16_t bgcolor)
{
  ...
  ST77xx_SetAddressWindow(x, y, (uint16_t)(x + font.FontWidth - 1), (uint16_t)(y + font.FontHeight - 1));
...
void ST77xx_DrawFilledCircle(int16_t x0, int16_t y0, int16_t r, uint16_t color)
{
  ...
  while (x < y)
  {
    ...
    ST77xx_DrawLine((uint16_t)(x0 - x), (uint16_t)(y0 + y), (uint16_t)(x0 + x), (uint16_t)(y0 + y), color);
    ST77xx_DrawLine((uint16_t)(x0 + x), (uint16_t)(y0 - y), (uint16_t)(x0 - x), (uint16_t)(y0 - y), color);

    ST77xx_DrawLine((uint16_t)(x0 + y), (uint16_t)(y0 + x), (uint16_t)(x0 - y), (uint16_t)(y0 + x), color);
    ST77xx_DrawLine((uint16_t)(x0 + y), (uint16_t)(y0 - x), (uint16_t)(x0 - y), (uint16_t)(y0 - x), color);
  }
}
